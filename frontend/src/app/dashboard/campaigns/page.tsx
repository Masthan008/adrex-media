...
import {
  Plus, Search, Target, MoreHorizontal, Megaphone, X
} from 'lucide-react';
...
export default function CampaignsPage() {
  ...
  const [newCamp, setNewCamp] = useState({ name: '', clientId: '', budget: '', status: 'DRAFT', startDate: '', endDate: '' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewCamp({ name: '', clientId: '', budget: '', status: 'DRAFT', startDate: '', endDate: '' });
        setShowModal(false);
      }
    } catch (error) {
            ...
            <tbody>
              <AnimatePresence>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-muted-foreground">No campaigns found.</td></tr>
                ) : filtered.map((c, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${st.color} ${st.bg}`}>
                          {st.label}
                        </span>
                      </td>
            ...
            <motion.div className="relative z-10 w-full max-w-lg glassmorphism rounded-2xl p-8"
              ...
              <form className="space-y-4" onSubmit={handleCreate}>
                <div>
                  ...
                  <input value={newCamp.name} onChange={e => setNewCamp(p => ({ ...p, name: e.target.value }))} type="text" placeholder="e.g. Summer Glow 2025" required className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5">Client</label>
                  <select value={newCamp.clientId} onChange={e => setNewCamp(p => ({ ...p, clientId: e.target.value }))} required className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={newCamp.budget} onChange={e => setNewCamp(p => ({ ...p, budget: e.target.value }))} type="number" placeholder="50000" required className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1.5">Status</label>
                    <select value={newCamp.status} onChange={e => setNewCamp(p => ({ ...p, status: e.target.value }))} className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={newCamp.startDate} onChange={e => setNewCamp(p => ({ ...p, startDate: e.target.value }))} type="date" required className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1.5">End Date</label>
                    <input value={newCamp.endDate} onChange={e => setNewCamp(p => ({ ...p, endDate: e.target.value }))} type="date" required className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all" />
                  </div>
                </div>
